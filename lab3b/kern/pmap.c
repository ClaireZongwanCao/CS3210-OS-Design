...
mem_init(void)
{
  ...
  // kern_pgdir wrong.
  lcr3(PADDR(kern_pgdir));boot_map_region(kern_pgdir,KSTACKTOP-KSTKSIZE,ROUNDUP(KSTKSIZE, PGSIZE),PADDR(bootstack),(PTE_W | PTE_P));
...
page_init(void)
{
      ...
      // free pages!
      size_t i;
      size_t start = IOPHYSMEM / PGSIZE;	
      size_t end = (PADDR(boot_alloc(0))) / PGSIZE;
      page_free_list = NULL;
      for (i = 0; i < npages; i++) {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  if (!(pgdir[va_pdx] & PTE_P)){
     if (create) {
        struct PageInfo *pp = page_alloc(ALLOC_ZERO);
        if (!pp) return NULL;
        pp->pp_ref++;
        pgdir[va_pdx] = page2pa(pp) | PTE_P | PTE_U | PTE_W;
     }
     else return NULL;
  }
  pte_t *p = KADDR(PTE_ADDR(pgdir[va_pdx]));
  return p+va_ptx;
}
